...
Optional auth: set `OTEL_INGEST_TOKEN` in `.env` and add `"OTEL_EXPORTER_OTLP_HEADERS": "Authorization=Bearer <token>"` to the settings snippet. Events carry `user.email`, so they join the same people you see everywhere else; times are UTC, so expect slight date skew for late-night work. Only configured machines send data — the dashboard shows coverage as the rollout ramps.
