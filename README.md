...
Optional auth: set `OTEL_INGEST_TOKEN` in `.env` and add `"OTEL_EXPORTER_OTLP_HEADERS": "Authorization=Bearer <token>"` to the settings snippet. Events carry `user.email`, so they join the same people you see everywhere else. Daily buckets use the Israel-local calendar day (`Asia/Jerusalem`), matching the Sun–Thu workweek the scores and streaks are built on, so work past midnight lands on the day you'd call it; hour buckets stay UTC and are converted for display. Rows ingested before this change are still keyed by UTC day. Only configured machines send data — the dashboard shows coverage as the rollout ramps.
