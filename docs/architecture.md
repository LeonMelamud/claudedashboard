...
All formulas live in `shared/src/scoring/` — executed on the server (`server/src/services/scoring.ts`) to build leaderboards, and imported by the web app for tooltips and the in-app Guide, so displayed explanations can't drift from computed numbers. Pipeline: per-user metrics → four axis scores (Adoption, Impact, Efficiency, Trust), each scored against fixed targets (`targets.ts`: per-workday rates × the range's workdays, √ curve capped at 100 in `normalize.ts`; admin-overridable via `settings.scoreTargets`) → composite score → segments (`scores.ts`, champion at ≥80/≥80) → badges, mostly percentile-based with small-sample reliability guards (`badges.ts`; a few are measured over history rather than the selected range — streaks, and Deep Diver's all-time compaction count — so an earned achievement doesn't vanish when the date picker narrows); metric/badge copy lives in `catalog.ts`. Axis scores are pure functions of the user's own row — no org max in any denominator, so one person's volume can't move anyone else's score. Git-derived Impact terms are coverage-gated: a term fewer than 25% of active users produced over a trailing-90d window (deliberately range-independent, so weights don't flip with the date picker — PR counting is GitHub-only) redistributes its weight to the kept terms. Nightly `score_snapshots` power Top Movers and celebration cards. Unit tests: `shared/test/scoring.test.ts`.
