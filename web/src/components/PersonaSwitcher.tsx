...
/**
 * Persona is a pure navigation preset — it changes the home route and nothing
 * else. Developer picks who they are; Team Lead picks their team.
 */
    ...
    <>
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content
            align="end"
            sideOffset={6}
            className="card pop-in z-50 w-52 p-1.5 shadow-xl"
          >
            ...
            {(Object.keys(PERSONA_LABELS) as Persona[]).map((p) => (
              <DropdownMenu.Item
                key={p}
                onSelect={() => choose(p)}
                className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-xs outline-none data-[highlighted]:bg-accent/15"
              >
                <span className="w-3.5">{persona === p && <Check size={12} />}</span>
                <span>
                  {PERSONA_LABELS[p]}
                  <span className="block text-[10px] text-muted">
                    {p === 'developer' && (email ?? 'home: your profile')}
                    {p === 'lead' && (teamId != null ? `team #${teamId}` : 'home: your team')}
            ...
            {persona === 'developer' && (
              <DropdownMenu.Item
                onSelect={() => setIdentifyOpen('developer')}
                className="mt-1 flex cursor-pointer items-center gap-2 rounded-md border-t border-border px-2 py-1.5 text-[11px] text-muted outline-none data-[highlighted]:bg-accent/15"
              >
                Change who I am…
              </DropdownMenu.Item>
            )}
      ...
      <IdentifyDialog
        ...
        onPickUser={(pickedEmail) => {
          ...
          navigate(`/user/${encodeURIComponent(pickedEmail)}`);
        }}
        onPickTeam={(id) => {
          setTeamId(id);
          setPersona('lead');
...
function IdentifyDialog({
  ...
  return (
    <Modal
      ...
      title={mode === 'lead' ? 'Which team do you lead?' : 'Who are you?'}
          ...
          <div className="max-h-72 space-y-0.5 overflow-y-auto">
            ...
            {users.map((u) => (
              <button
                ...
                className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm hover:bg-accent/10"
              >
        ...
        <div className="max-h-72 space-y-0.5 overflow-y-auto">
          ...
          {(teamsQ.data?.teams ?? []).map((t) => (
            <button
              ...
              className={cn('flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm hover:bg-accent/10')}
            >
